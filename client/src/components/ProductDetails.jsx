    ...
    <div className='flex mx-[8%]'>
      ...
      <div className='flex flex-col w-1/2 gap-4'>
        ...
        <div className='flex flex-col gap-2'>
          ...
          <div className='flex gap-4'>
            {colors.map((color, index) => (
              <button className={`rounded-full w-8 h-8 mr-2 bg-[${color}] ${selectedColor === index ? 'border-2 border-black' : ''}`} key={index} onClick={() => { setSelectedColor(index) }}></button>
            ))}
          </div>
        </div>
        <hr />
        ...
        <div className='flex justify-between'>
          ...
          <div className='flex bg-black rounded-full text-white px-5 py-3 w-2/3 items-center justify-center'>Add to Cart</div>
        </div>
      </div>
    </div>
  )
